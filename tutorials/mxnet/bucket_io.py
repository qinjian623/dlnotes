...
def default_build_vocab(path):
    ...
    the_vocab = {}
    # put a dummy element here so that len(vocab) is correct
    the_vocab[' '] = 0
    for word in content:
        if len(word) == 0:
            continue
...
def default_gen_buckets(sentences, batch_size, the_vocab):
    ...
    tl = 0
    buckets = []
    # TODO: There are better heuristic ways to do this
...
class BucketSentenceIter(mx.io.DataIter):
    ...
    def make_data_iter_plan(self):
        ...
        for i, n in enumerate(bucket_n_batches):
            print(i, int(n))
        bucket_plan = np.hstack([np.zeros(int(n), int)+i
                                 for i, n in enumerate(bucket_n_batches)])
        ...
        for i_bucket in self.bucket_plan:
            ...
            if self.model_parallel:
                if self.data[i_bucket][:, idx].shape[1] == 0:
                    print("WARNING: detected shape "
                          + str(self.data[i_bucket][:, idx].shape))
                    continue
